...
The frequently used word 'call' in this file refers to the name of the function. For example the following:
...
class Function:
    ...
    score: int | None = None  # add this to record the score of the function.
    global_sample_nums: int | None = None  # record the order of the current program in the sequence of samples taken.
    sample_time: float | None = None  # add this
    evaluate_time: float | None = None  # add this
    ...
    def __str__(self) -> str:
        ...
        if self.docstring:
            ...
            # Here, we assume the indentation is always two spaces. =>  two spaces is ugly, change to four
            # two spaces is ugly, change to four spaces.
            new_line = '\n' if self.body else ''
    ...
    def visit_FunctionDef(self,  # pylint: disable=invalid-name
        ...
        if node.col_offset == 0:  # We only care about first level functions.
            ...
            self._functions.append(Function(
                name=node.name,
                args=ast.unparse(node.args),  # ast.unparse() failed in Python3.8. Let us try Python3.9
